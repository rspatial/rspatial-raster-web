...
Note that to get the number of spatial object ('features'), of SpatVector `p`, you need can use `nrow(p)`, or `size(p)`. You can also do `length(p)` but that returns the length of the spatial objects (either zero for points the lenght of the lines, or the perimeter of the polygons).
