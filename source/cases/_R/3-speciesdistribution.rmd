...
if (!file.exists('data/WILDPOT.txt')) {
	download.file('https://biogeo.ucdavis.edu/data/rspatial/wildpot.zip', dest = 'data/wildpot.zip')
	unzip('data/wildpot.zip', exdir='data')
}
```
...
```{r, a1b}
d <- read.table('data/WILDPOT.txt', header=TRUE)
```
...
d <- readLines('data/WILDPOT.txt', encoding='UTF-8')
...
if (!file.exists("data/pt_countries.shp")) {
	download.file('https://biogeo.ucdavis.edu/data/rspatial/pt_countries.zip', 'data/pt_countries.zip')
	unzip('data/pt_countries.zip',exdir="data")
}


library(raster)
cn <- shapefile('data/pt_countries.shp')
